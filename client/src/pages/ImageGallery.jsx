...
const ImageGallery = () => {
  ...
  const { data: images } = useFetchData(galleryURL);

  useEffect(() => {
    if (images && images.data) {
      const uniqueCategories = ['ALL', ...new Set(images.data.map(item => item.category.toUpperCase()))];
  ...
  useEffect(() => {
    if (Array.isArray(images?.data)) {
      ...
      const filtered = selectedCategory === 'ALL'
        ? sortedImages
        : sortedImages.filter((image) => image.category.toUpperCase() === selectedCategory);
